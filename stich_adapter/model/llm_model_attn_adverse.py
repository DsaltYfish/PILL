...
class Attention(nn.Module):
    ...
    def forward(self, 
        ...
        # txt_token_attn, _ = modal_attn.max(3)
        # txt_token_label = torch.zeros_like(txt_token_attn)
        # txt_token_loss = F.binary_cross_entropy_with_logits(txt_token_attn, txt_token_label)

        # vis_token_attn, _ = modal_attn.max(2)
        # vis_token_label = torch.ones_like(vis_token_attn)
        # vis_token_loss = F.binary_cross_entropy_with_logits(vis_token_attn, vis_token_label)
        ...
        return self.wo(output_l), imp_loss, modal_attn
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, 
        ...
        h, loss, attn = self.attention.forward(x_a+x_norm, start_pos, freqs_cis, mask, previous_attn)
    ...
    def forward(self, examples, labels,example_mask, images=None, prefix_img=None, prefix_nonimg=None,img_indicators=None):
        ...
        for idx, layer in enumerate(self.layers):
            if self.grad_checkpoint and self.training:
                ...
                h, attn, loss = layer(h, start_pos, freqs_cis, mask, image_positions, attn)
            ga_loss += loss
