...
class ReversePath(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.reverse_prob = args.reverse_prob

    def forward(self, x, y):
        return self.reverse_path(x, y, self.reverse_prob)

    def reverse_path(self, x, y, reverse_prob: float = 0.):
        if reverse_prob == 0. or not self.training:
            return x, y
        if reverse_prob > random.random():
            return y, x
        else:
            return x, y
...
class Transformer(nn.Module):
    def __init__(self, params: ModelArgs):
        ...
        self.itm_criterion = torch.nn.BCEWithLogitsLoss()
        ...
        self.adapter_itm_token = nn.Parameter(torch.empty(1, 1, params.dim))
        self.adapter_itm_head = nn.Linear(params.dim, 1, False)
    ...
    def insert_image_embeds(self,examples,labels, image_embeds,prefix_img,prefix_nonimg,img_indicators):
        ...
        for i, (example,label) in enumerate(zip(examples,labels)):
            image_position = torch.zeros([seqlen+1, dim], dtype=torch.bool)
            new_example=torch.cat([image_embeds[i],example],0)
            new_label=torch.cat([torch.zeros(image_embeds.shape[1]).to(examples.device).type_as(labels),
                                label])
            new_example = new_example[:seqlen]
            new_label = new_label[:seqlen]
            image_position[:image_embeds.shape[1]] = True
    ...
    def forward(self, examples, labels,example_mask, images=None, prefix_img=None, prefix_nonimg=None,img_indicators=None):
        ...
        adapter_itm_token = self.adapter_itm_token.repeat(_bsz, 1, 1)
        h = torch.cat((h, adapter_itm_token), dim=1)
        ...
        for idx, layer in enumerate(self.layers):
            if self.grad_checkpoint and self.training:
                ...
                h, loss = torch.utils.checkpoint.checkpoint(
                ...
                h, loss = layer(h, start_pos, freqs_cis, mask, image_positions)
            mse_loss += loss

        h = self.norm(h)
        output = self.output(h)
        # output = output[:, :-1, :].reshape(-1, self.vocab_size)
        output = output[:, :-2, :].reshape(-1, self.vocab_size) # -2 为了计算itm
        labels = labels[:, 1:].flatten()

        output_itm = self.adapter_itm_head(h[:, -1, :])[:, 0]
        itm_loss = self.itm_criterion(output_itm, img_indicators.float())

        c_loss = self.criterion(output, labels)
        
        return c_loss, itm_loss
