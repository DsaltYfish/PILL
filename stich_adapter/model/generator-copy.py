...
class Generator:
    ...
    def generate(
        ...
        image_embeds=self.model.visual_feature_extractor.forward_image(images).half()
        image_embeds=self.model.adapter_proj(image_embeds)

        n_feats = image_embeds.shape[1]

        prompt_tokens=[]
        image_positions=[]
        # for i,x in enumerate(prompts):
        #     image_position = torch.zeros([params.max_seq_len, params.dim], dtype=torch.bool)
        #     if indicators[i]==1:
        #         token_idx=prefix_img_token+[0]*n_feats+self.tokenizer.encode(x, bos=False, eos=False)
        #         image_position[len(prefix_img_token):len(prefix_img_token)+n_feats] = True
        #     else:
        ...
        for i,x in enumerate(prompts):
            image_position = torch.zeros([params.max_seq_len, params.dim], dtype=torch.bool)
            # token_idx=[0]*n_feats+self.tokenizer.encode(x, bos=True, eos=False)
            # image_position[:n_feats] = True
            token_idx=prefix_img_token+[0]*n_feats+self.tokenizer.encode(x, bos=False, eos=False)
            image_position[len(prefix_img_token):len(prefix_img_token)+n_feats] = True
            prompt_tokens.append(token_idx)
            image_positions.append(image_position.unsqueeze(0))
        
        image_positions = torch.cat(image_positions, 0).cuda()
