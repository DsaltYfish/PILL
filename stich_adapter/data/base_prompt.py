...
def get_choice_text(probelm, options):
    ...
    #print(choice_txt)
    return choice_txt, len(choices)


def get_answer(problem, options, num_choice, random_select_rate):
    # if random.random() < random_select_rate:
    #     return options[random.randint(0, num_choice-1)]
    # else:
    return options[problem['answer']]#原始代码中只有这行
...
def build_prompt(problems, test_qid, args):
    ...
    # choice = get_choice_text(problems[test_qid], args.options)
    # answer = get_answer(problems[test_qid], args.options)
    choice, num_choice = get_choice_text(problems[test_qid], args.options)
    answer = get_answer(problems[test_qid], args.options, num_choice, args.random_select_rate)
    lecture = get_lecture_text(problems[test_qid])
    solution = get_solution_text(problems[test_qid])
